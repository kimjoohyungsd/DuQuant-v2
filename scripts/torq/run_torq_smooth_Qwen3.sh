...
# ============================================================
# TORQ (arXiv:2605.19561) + SmoothQuant : Qwen3 models only
# ============================================================
# Same shape as run_torq_Qwen3.sh, TWO arguments added:
#   + --smooth --alpha 0.6
# (see run_torq_smooth.sh's header for what that does: per-channel
# SmoothQuant scale applied in UniformAffineQuantizer.forward() before
# TORQ's rotation, same alpha=0.6 convention as ../duquantpp/.)
#
...
#              bash scripts/torq/run_torq_smooth_Qwen3.sh
#   2) fallback (QWEN3_PYTHON unset): the shared flatquant python, with
#      PYTHONPATH=.vendor/transformers_qwen3 prepended (see
#      ../../.vendor/README.md).
#
# Otherwise identical to run_torq_smooth.sh: one GPU per model, full
# stdout+stderr captured per run under log/torq/, plus --results_json,
# tagged "torq_smooth" so it doesn't collide with run_torq_Qwen3.sh's
# "torq" files in the same log directory.
#
#   bash scripts/torq/run_torq_smooth_Qwen3.sh
#   QWEN3_PYTHON=/path/to/duquant-qwen3/bin/python bash scripts/torq/run_torq_smooth_Qwen3.sh
...
# --- TORQ + SmoothQuant, Qwen3 models, one GPU each, in parallel ---
